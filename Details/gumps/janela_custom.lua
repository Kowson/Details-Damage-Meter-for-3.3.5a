...
--> create the window
	...
	function _details:CloseCustomDisplayWindow()
	...
	function _details:OpenCustomDisplayWindow()
		...
		if (not _G.DetailsCustomPanel) then
			...
			custom_window:SetScript("OnMouseDown", function(self, button)
				...
				elseif (button == "RightButton") then
					if (not self.moving) then
						_details:CloseCustomDisplayWindow()
					end
				end
			end)
			...
			function DetailsCustomPanel:RemoveDisplay(custom_object, index)
				...
				_details:ResetCustomFunctionsCache()
			end
			...
			function DetailsCustomPanel:CreateNewCustom()
				...
				if (string.len(name) < 5) then
					return false, _details:Msg(Loc["STRING_CUSTOM_SHORTNAME"])
				elseif (string.len(name) > 32) then
					return false, _details:Msg(Loc["STRING_CUSTOM_LONGNAME"])
				end
				...
				if (DetailsCustomPanel.BoxType == 1) then
					...
					if (DetailsCustomPanel.IsEditing) then
						...
						if (DetailsCustomPanel.IsImporting) then
							_details:Msg(Loc["STRING_CUSTOM_IMPORTED"])
						else
							_details:Msg(Loc["STRING_CUSTOM_SAVED"])
						end
						
						if (DetailsCustomPanel.IsImporting) then
							tinsert(_details.custom, object)
						end
						...
						_setmetatable(new_custom_object, _details.attribute_custom)
						new_custom_object.__index = _details.attribute_custom
						_details:Msg(Loc["STRING_CUSTOM_CREATED"])
					end
			...
			function custom_window:CreateMenuButton(label, icon, clickfunc, param1, param2, tooltip, name, coords)
				...
				circle:SetTexture("Interface\\AddOns\\Details\\images\\AlternateForm")
			...
			custom_window:CreateMenuButton(Loc["STRING_CUSTOM_EDIT"], "Interface\\AddOns\\Details\\images\\INV_Inscription_RunescrollOfFortitude_Red", build_menu, start_edit, nil, nil, "Edit", {0.07, 0.93, 0.07, 0.93}) --> localize
			...
			local export_display = function(_, _, custom_object, index)
				...
				local serialized_table = _details:Serialize(export_object)
				--local zip = LibStub:GetLibrary("LibCompress"):CompressHuffman(serialized_table)
				--local encoded = _details._encode:Encode(zip)
				local encoded = _details._encode:Encode(serialized_table)
				
				if (not custom_window.ExportBox) then
					local editbox = _details.gump:NewTextEntry(custom_window, nil, "$parentExportBox", "ExportBox", 842, 20)
			...
			local import_display = function(_, _, custom_object, index)
				...
				if (not custom_window.ImportBox) then
					...
					local editbox = _details.gump:NewTextEntry(custom_window, nil, "$parentImportBox", "ImportBox", 772 - export_string.width - 2, 20)
					...
					local import = function()
						...
						--local deserialized_object = select(2, _details:Deserialize(unzip))
						...
						local deserialized_object = select(2, _details:Deserialize(decode))
						...
						setmetatable(deserialized_object, _details.attribute_custom)
						deserialized_object.__index = _details.attribute_custom
						
						_details.MicroButtonAlert.Text:SetText(Loc["STRING_CUSTOM_IMPORT_ALERT"])
						_details.MicroButtonAlert:SetPoint("bottom", custom_window.box0.acceptbutton.widget, "top", 0, 20)
			...
			local attributes = {
				...
				{icon =[[Interface\AddOns\Details\images\INV_Inscription_Scroll]], label = Loc["STRING_CUSTOM_ATTRIBUTE_SCRIPT"], box = 2, attribute = false, boxtype = 2},
			...
			selected_left:SetTexture([[Interface\AddOns\Details\images\Store-Main]])
			...
			selected_right:SetTexture([[Interface\AddOns\Details\images\Store-Main]])
			...
			selected_center:SetTexture([[Interface\AddOns\Details\images\Store-Main]])
			...
			for i = 1, 10 do
				...
				button.icon:SetTexture([[Interface\AddOns\Details\images\custom_icons]])
			...
			--> create box type 1
				...
				--source
					...
					local source_icon =[[Interface\AddOns\Details\images\Indicator-Yellow]]
					...
					adds_boss_image:SetTexture("Interface\\AddOns\\Details\\images\\UI-MicroButton-Raid-Up")
					...
					modelFrameBackground:SetTexture([[Interface\AddOns\Details\images\UI-GuildAchievement-Parchment-Horizontal-Desaturated]])
					...
					local buttonMouseOver = function(button)
						...
						local bgImage = "Interface\\LFGFrame\\UI-LFG-BACKGROUND-Ulduar"
					...
					local EncounterSelect = function(_, _, instanceId, bossIndex, ej_id)
						...
						local encounterID = _details:GetEncounterIdFromBossIndex(instanceId, bossIndex)
						
						if (encounterID) then
							local actors = _details:GetEncounterActorsName(encounterID)
							...
							for actor, actorTable in pairs(actors) do 
								...
								if (not thisButton) then
									...
									t:SetTexture([[Interface\AddOns\Details\images\Target]])
									...
									local border = gump:NewImage(thisButton, "Interface\\AddOns\\Details\\images\\Spellbook-Parts", 40, 38, nil, nil, "border", "DetailsCustomActors2EncounterBorderButton"..i)
									...
									local line = gump:NewImage(thisButton, "Interface\\AddOns\\Details\\images\\Spellbook-Parts", 134, 25, nil, nil, "line", "DetailsCustomActors2EncounterLineButton"..i)
					...
					local BuildEncounterMenu = function()
						...
						for instanceId, instanceTable in pairs(_details.EncounterInformation) do 
						
							if (_details:InstanceIsRaid(instanceId)) then
								...
								for index, encounterName in ipairs(instanceTable.boss_names) do 
									...
									local L, R, T, B, Texture = _details:GetBossIcon(instanceId, index)
				...
				--target
					...
					local target_icon =[[Interface\AddOns\Details\images\Indicator-Yellow]]
					local target_icon2 =[[Interface\AddOns\Details\images\Indicator-Gray]]
						...
						if (encounterID) then
							...
							for actor, actorTable in pairs(actors) do 
								...
								if (not thisButton) then
									...
									local border = gump:NewImage(thisButton, "Interface\\AddOns\\Details\\images\\Spellbook-Parts", 40, 38, nil, nil, "border", "DetailsCustomActorsEncounterBorderButton"..i)
									border:SetTexCoord(0.00390625, 0.27734375, 0.44140625,0.69531250)
									border:SetDrawLayer("background")
									border:SetPoint("topleft", thisButton.button, "topleft", -9, 9)
									
									local line = gump:NewImage(thisButton, "Interface\\AddOns\\Details\\images\\Spellbook-Parts", 84, 25, nil, nil, "line", "DetailsCustomActorsEncounterLineButton"..i)
									line:SetTexCoord(0.31250000, 0.96484375, 0.37109375, 0.52343750)
									line:SetDrawLayer("background")
									line:SetPoint("left", thisButton.button, "right", -110, -3)
				...
				--spellid
					...
					spell_id_boss_image:SetTexture("Interface\\AddOns\\Details\\images\\UI-MicroButton-Raid-Up")
					...
					local buttonMouseOver = function(button)
						...
						GameTooltip:SetOwner(button, "ANCHOR_TOPLEFT")
						GameTooltip:SetHyperlink("spell:"..button.MyObject.spellid)
						GameTooltip:Show()
					end
					local buttonMouseOut = function(button)
						button.MyObject.image:SetBlendMode("BLEND")
					...
					local EncounterSelect = function(_, _, instanceId, bossIndex)
						...
						local spells = _details:GetEncounterSpells(instanceId, bossIndex)
						...
						for spell, _ in pairs(spells) do 
							...
							if (not thisButton) then
								...
								local border = gump:NewImage(thisButton, "Interface\\AddOns\\Details\\images\\Spellbook-Parts", 40, 38, nil, nil, "border", "DetailsCustomEncounterBorderButton"..i)
								border:SetTexCoord(0.00390625, 0.27734375, 0.44140625,0.69531250)
								border:SetDrawLayer("background")
								border:SetPoint("topleft", thisButton.button, "topleft", -9, 9)
								
								local line = gump:NewImage(thisButton, "Interface\\AddOns\\Details\\images\\Spellbook-Parts", 84, 25, nil, nil, "line", "DetailsCustomEncounterLineButton"..i)
								line:SetTexCoord(0.31250000, 0.96484375, 0.37109375, 0.52343750)
								line:SetDrawLayer("background")
							...
							local name_spell, _, icon_spell = _GetSpellInfo(spell)
							thisButton.image:SetTexture(icon_spell)
							thisButton.label:SetText(name_spell)
