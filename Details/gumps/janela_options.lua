...
function _details:OpenOptionsWindow(instance, no_reopen, section)
...
-- Details Overall -------------------------------------------------------------------------------------------------------------------------------------------------
	...
	--> create bars
		...
		local fillbars = g:NewButton(window, _, "$parentCreateExampleBarsButton", nil, 110, 14, _details.CreateTestBars, nil, nil, nil, Loc["STRING_OPTIONS_TESTBARS"], 1)
	...
	--> change log
		...
		local changelog = g:NewButton(window, _, "$parentOpenChangeLogButton", nil, 110, 14, _details.OpenNewsWindow, nil, nil, nil, Loc["STRING_OPTIONS_CHANGELOG"], 1)
	...
	--> send feedback
		...
		local feedback_button = g:NewButton (window, _, "$parentOpenFeedbackButton", nil, 110, 14, _details.OpenFeedbackWindow, nil, nil, nil, Loc["STRING_OPTIONS_SENDFEEDBACK"], 1)
...
function window:CreateFrame18()
	...
	--> Use Scroll Bar
		...
		--
		g:NewSwitch(frame18, _, "$parentUseScrollSlider", "scrollSlider", 60, 20, _, _, _details.use_scroll)
		...
		frame18.scrollSlider.OnSwitch = function(self, _, value) --> slider, fixedValue, sliderValue
			...
			_details:instanceCallFunction(_details.UpdateSegments) -- update instance.showing for newly created tables
			_details:instanceCallFunction(_details.UpdateSoloMode_AfertReset) -- verify if you need to reset the solo mode window table
			_details:instanceCallFunction(_details.ResetGump) --details:ResetGump("of all instances")
			_details:UpdateGumpMain(-1, true) -- update all instances
		end
...
function window:CreateFrame1()
	...
	--> wheel speed
		...
		--
		local s = g:NewSlider(frame1, _, "$parentWheelSpeedSlider", "WheelSpeedSlider", SLIDER_WIDTH, 20, 1, 3, 1, _details.scroll_speed)
		s:SetBackdrop(slider_backdrop)
		s:SetBackdropColor(unpack(slider_backdrop_color))
		s:SetThumbSize(50)
	...
	--> Max Instances
		...
		--
		local s = g:NewSlider(frame1, _, "$parentSliderMaxInstances", "maxInstancesSlider", SLIDER_WIDTH, 20, 1, 30, 1, _details.instances_amount) -- min, max, step, defaultv
		s:SetBackdrop(slider_backdrop)
		s:SetBackdropColor(unpack(slider_backdrop_color))
		s:SetThumbSize(50)
...
function window:CreateFrame6()
	...
	--> micro displays selection
			...
			local OnMicroDisplaySelect = function(_, instance, micro_display)
				...
				if (index == -1) then
					return _details.StatusBar:SetPlugin(instance, -1, anchor)
				end

				local absolute_name = _details.StatusBar.Plugins[index].real_name
				_details.StatusBar:SetPlugin(instance, absolute_name, anchor)
			...
			local BuildLeftMicroMenu = function()
				...
				for index, _name_and_icon in ipairs(_details.StatusBar.Menu) do
			...
			local HideLeftMicroFrameButton = g:NewButton(frame6.MicroDisplayLeftDropdown, _, "$parentHideLeftMicroFrameButton", "HideLeftMicroFrameButton", 22, 22, function(_, _, self)
				if (_G.DetailsOptionsWindow.instance.StatusBar["left"].options.isHidden) then
					_details.StatusBar:SetPlugin(_G.DetailsOptionsWindow.instance, _G.DetailsOptionsWindow.instance.StatusBar["left"].real_name, "left")
				else
					_details.StatusBar:SetPlugin(_G.DetailsOptionsWindow.instance, -1, "left")
				end
				if (_G.DetailsOptionsWindow.instance.StatusBar["left"].options.isHidden) then
			...
			local HideCenterMicroFrameButton = g:NewButton(frame6.MicroDisplayCenterDropdown, _, "$parentHideCenterMicroFrameButton", "HideCenterMicroFrameButton", 22, 22, function(_, _, self)
				if (_G.DetailsOptionsWindow.instance.StatusBar["center"].options.isHidden) then
					_details.StatusBar:SetPlugin(_G.DetailsOptionsWindow.instance, _G.DetailsOptionsWindow.instance.StatusBar["center"].real_name, "center")
				else
					_details.StatusBar:SetPlugin(_G.DetailsOptionsWindow.instance, -1, "center")
				end
			...
			local HideRightMicroFrameButton = g:NewButton(frame6.MicroDisplayRightDropdown, _, "$parentHideRightMicroFrameButton", "HideRightMicroFrameButton", 20, 20, function(_, _, self)
				if (_G.DetailsOptionsWindow.instance.StatusBar["right"].options.isHidden) then
					_details.StatusBar:SetPlugin(_G.DetailsOptionsWindow.instance, _G.DetailsOptionsWindow.instance.StatusBar["right"].real_name, "right")
				else
					_details.StatusBar:SetPlugin(_G.DetailsOptionsWindow.instance, -1, "right")
				end
				if (_G.DetailsOptionsWindow.instance.StatusBar["right"].options.isHidden) then
					self:GetNormalTexture():SetDesaturated(false)
				else
...
--			/dump _details:GetInstance(1).StatusBar ["left"].real_name
	--> show side bars
...
function window:CreateFrame11()
	...
	--interrupts
		...
		local channel_list = {
			{value = "PRINT", icon = [[Interface\FriendsFrame\UI-Toast-ToastIcons]], iconsize = {14, 14}, texcoord = {0.0390625, 0.203125, 0.09375, 0.375}, label = Loc["STRING_CHANNEL_PRINT"], onclick = on_select_channel},
	...
	--deaths
		...
		local on_select_channel = function(self, _, channel)
			_details.announce_deaths.where = channel
		end
		...
		local channel_list = {
			...
			{value = 4, icon = [[Interface\FriendsFrame\UI-Toast-ToastIcons]], iconsize = {14, 14}, texcoord = {0.0390625, 0.203125, 0.09375, 0.375}, iconcolor = {1, 1, 1, 1}, label = Loc["STRING_CHANNEL_PRINT"], onclick = on_select_channel},
		...
		local d = g:NewDropDown(frame11, _, "$parentDeathChannelDropdown", "DeathChannelDropdown", DROPDOWN_WIDTH, 20, build_channel_menu, _details.announce_deaths.where)
		d.onenter_backdrop = dropdown_backdrop_onenter
		d.onleave_backdrop = dropdown_backdrop_onleave
		d:SetBackdrop(dropdown_backdrop)
		d:SetBackdropColor(unpack(dropdown_backdrop_onleave))
...
function window:update_all(editing_instance, section)
	...
	_G.DetailsOptionsWindow1WheelSpeedSlider.MyObject:SetValue(_details.scroll_speed)
	...
	for _, slider in ipairs(window.plugin_widgets) do
		local plugin_stable = _details:GetPluginSavedTable(slider.PluginName)
	...
	_G.DetailsOptionsWindow18ReportFormatDropdown.MyObject:Select(_details.report_schema)
	--disabled groups
	...
	_G.DetailsOptionsWindow18UseScrollSlider.MyObject:SetValue(_details.use_scroll)
	...
	_G.DetailsOptionsWindow20TooltipTextColorPickRight.MyObject:SetColor(unpack(_details.tooltip.fontcolor_right))
	_G.DetailsOptionsWindow20TooltipTextColorPickAnchor.MyObject:SetColor(unpack(_details.tooltip.header_text_color))
