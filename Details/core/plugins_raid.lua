...
--> built in announcers
	...
	function _details:interrupt_announcer(token, time, src_serial, src_name, src_flags, dst_serial, dst_name, dst_flags, spellid, spellname, spelltype, extraSpellID, extraSpellName, extraSchool)
		...
		if (channel ~= "PRINT" and src_name == _details.playername) then
		...
		elseif (channel == "PRINT") then
			...
			if (custom ~= "") then
				custom = custom:gsub("{spell}", spellname)
				custom = custom:gsub("{next}", src_name)
				_details:SendMsgToChannel (custom, "PRINT")
			else
				local minute, second = _details:GetCombat():GetFormatedCombatTime()

				local _, class = _UnitClass(src_name)
				...
				local msg = "|cFFFFFF00[|r".. minute ..  ":" .. second .. "|cFFFFFF00]|r Interrupt: " .. spellname .. " (" .. class_color .. _details:GetOnlyName(src_name) .. "|r)"
				_details:SendMsgToChannel(msg, "PRINT")
			end
		end
	end
	...
	function _details:cooldown_announcer(token, time, src_serial, src_name, src_flags, dst_serial, dst_name, dst_flags, spellid, spellname)
		...
		if (channel ~= "PRINT" and src_name == _details.playername) then -- and _details.announce_cooldowns.enabled
		...
		elseif (channel == "PRINT") then
			...
			if (src_name == dst_name and src_name ~= _details.playername) then
				return
			end
			...
			local _, class2 = _UnitClass(dst_name)
			...
			msg = "|cFF8F8FFF[|r".. minute ..  ":" .. second .. "|cFF8F8FFF]|r Cooldown: " .. spellname .. " (" .. class_color .. _details:GetOnlyName(src_name) .. "|r" .. class_color2 .. dst_name .. "|r)"
	...
	function _details:death_announcer(token, time, src_serial, src_name, src_flags, dst_serial, dst_name, dst_flags, death_table, last_cooldown, death_at, max_health)
		--if (_details.announce_deaths.enabled) then
			...
			if (where == 4) then --> observer
				local _, class = _UnitClass(dst_name)
				local class_color = "|cFFFFFFFF"
				...
				msg = _cstr(Loc["STRING_OPTIONS_RT_DEATH_MSG"], dst_name) .. ":"
			end
			local spells = ""
