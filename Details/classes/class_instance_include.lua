...
function _details:ResetInstanceConfig(maintainsnap)
...
function _details:LoadInstanceConfig()
	for key, value in pairs(_details.instance_defaults) do 
		if (self[key] == nil) then
			if (type(value) == "table") then
				self[key] = table_deepcopy(_details.instance_defaults[key])
			else
				self[key] = value
			end
...
_details.instance_defaults = {
	...
	--instance button info
		instancebutton_config = {size = {20, 16}, anchor = {2, 0}, normal_texture =[[Interface\Buttons\UI-Panel-MinimizeButton-Highlight]], textcolor = {1, 1, 1, 1}, textsize = 11, textfont = "Friz Quadrata TT", textshadow = false},
	--close button info
