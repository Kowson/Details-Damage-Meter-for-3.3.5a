...
-- enabled
	...
	function SliderMetaFunctions:Enable()
		...
		if (not self.lock_texture) then
			gump:NewImage(self,[[Interface\AddOns\Details\images\PetBattle-LockIcon]], 12, 12, "overlay", {0.0546875, 0.9453125, 0.0703125, 0.9453125}, "lock_texture", "$parentLockTexture")
...
--> scripts
	...
	f:Hide()
	--f:SetBackdrop({bgFile =[[Interface\AddOns\Details\images\UI-DialogBox-Background-Dark]], tile = true, tileSize = 5})
	f:SetHeight(18)
	...
	function SliderMetaFunctions:TypeValue()
		if (not self.isSwitch) then
			...
			if (not SliderMetaFunctions.editbox_typevalue) then
				...
				editbox:SetBackdrop({bgFile =[[Interface\AddOns\Details\images\UI-DialogBox-Background-Dark]],
...
--> build frames
	...
	local text = slider:CreateFontString(nil, "overlay", "GameFontNormalSmall")
...
--> defaults	
	...
	SliderObject.amt = SliderObject.slider:CreateFontString(nil, "overlay", "GameFontNormalSmall")
