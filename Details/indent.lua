...
-- new as of lua 5.1
   ...
   -- returns false or: true, nextPos, ewhichsCount
   ...
   local function nextBracketString(text, pos, ewhichsCount)
      local state = 0
      while true do
	 local byte = stringbyte(text, pos)
	 if not byte then
	 ...
	 if byte == bytes.BYTE_RIGHTBRACKET then
	    if state == 0 then
	       state = 1
	    elseif state == ewhichsCount + 1 then
   ...
   local function nextComment(text, pos)
      ...
      local isBracketString, nextPos, ewhichsCount = isBracketStringNext(text, pos)
      if isBracketString then
	 local tokenType, nextPos2 = nextBracketString(text, nextPos, ewhichsCount)
   ...
   function nextToken(text, pos)
	 ...
	 if byte == bytes.BYTE_LEFTBRACKET then
	    local isBracketString, nextPos, ewhichsCount = isBracketStringNext(text, pos)
	    if isBracketString then
	       return nextBracketString(text, nextPos, ewhichsCount)
	    else
