...
--> Main Plugin Object
local DpsTuningPlugin = _details:NewPluginObject("Details_DpsTuning")
--> Main Frame
...
local _GetSpellInfo =_details.getspellinfo --> details api local
...
local function CreatePluginFrames()
	...
	--> get the framework
	local fw = _details:GetFramework()
	...
	local string_player_damage_done =[[
		...
		-- the first thing to do is get the combat, so, we use here the command "_details:GetCombat( "overall" "current" or "segment number")"
		...
		local my_self = current_combat(DETAILS_ATTRIBUTE_DAMAGE, _details.playername)
		
		-- _details.playername holds the name of your character.
	...
	function DpsTuningPlugin:OnDetailsEvent(event, ...)
		...
		if (event == "SHOW") then --> plugin shown on screen, actived
			...
			if (not DpsTuningPlugin.frames_created) then
				...
				--tricky, localize members inside the plugin, so it doesn't need to lookup on _details object every time
	...
	function DpsTuningPlugin.GetActivityTime(thisspell, time)
		if (thisspell.time_end) then --> o time do player this trancado
			local t = thisspell.time_end - thisspell.start
			if (t < 6) then
				t = 6
			end
			return t
	...
	function DpsTuningPlugin.RefreshSpells()
		...
		elseif (DpsTuningPlugin.db.SpellBarsShowType == 2) then --> player activity dps
			...
			local player_time = DpsTuningPlugin.CurPlayer:Time()
			local i = 0
	...
	function DpsTuningPlugin:BuildHeader()
		...
		local icon =[[Interface\AddOns\Details\images\friendship-FistOrc]]
	...
	local aura_onenter = function(self, capsule)
		...
		if (buff) then
			...
			local minutes, seconds = floor(buff.uptime/60), floor(buff.uptime%60)
			GameCooltip:AddLine("Uptime:", minutes .. "m " .. seconds .. "s", 1, "white", nil, 10, "Arrial Narrow")
			GameCooltip:AddLine("Percent:", _cstr("%.1f", buff.uptime/DpsTuningPlugin.CurPlayer:Time()*100) .. "%", 1, "white", nil, 10, "Arrial Narrow")
	...
	function DpsTuningPlugin:AuraApplied(time, src_serial, src_name, src_flags, dst_serial, dst_name, dst_flags, spellid, spellname, spellschool, type, amount)
		if (type == "BUFF") then
	...
	function DpsTuningPlugin:AuraRefresh(time, src_serial, src_name, src_flags, dst_serial, dst_name, dst_flags, spellid, spellname, spellschool, type, amount)
		if (type == "BUFF") then
			local buff_table = buff_activity[spellid]
			if (buff_table) then
				if (buff_table.actived_at and buff_table.actived) then
	...
	function DpsTuningPlugin:AuraRemoved(time, src_serial, src_name, src_flags, dst_serial, dst_name, dst_flags, spellid, spellname, spellschool, type, amount)
		if (type == "BUFF") then
			local buff_table = buff_activity[spellid]
			if (buff_table) then
				if (buff_table.actived_at and buff_table.actived) then
	...
	local bar_onenter_script = function(self, capsule)
		...
		GameCooltip:AddLine("Dps:", DpsTuningPlugin:ToK2(floor(spell.total / DpsTuningPlugin.CurPlayer:Time())), 1, "white", nil, 10, "Arrial Narrow")
		...
		if (misc) then
			...
			if (debuff_uptime) then
				...
				if (this_spell) then
					...
					local minutes, seconds = floor(uptime/60), floor(uptime%60)
					GameCooltip:AddLine("Uptime:", minutes .. "m " .. seconds .. "s", 1, "white", nil, 10, "Arrial Narrow")
				end
			end
		end
	...
	function DpsTuningPlugin:UpdateSummary()
		if (DpsTuningPlugin.CurPlayer) then
			...
			DpsTuningPlugin.activity_dps.text = DpsTuningPlugin:ToK2(floor(DpsTuningPlugin.CurPlayer.total / DpsTuningPlugin.CurPlayer:Time()))
...
function DpsTuningPlugin:OnEvent(_, event, ...)
	...
	if (event == "COMBAT_LOG_EVENT_UNFILTERED") then
		...
		local time1, token, hidding, src_serial, src_name, src_flags, src_flags2, dst_serial, dst_name, dst_flags, dst_flags2, spellid, spellname, spellschool, type, amount = select(1, ...)
		
		if (src_name == DpsTuningPlugin.playername and dst_name == DpsTuningPlugin.playername) then
			if (token == "SPELL_AURA_APPLIED") then
				DpsTuningPlugin:AuraApplied(time1, src_serial, src_name, src_flags, dst_serial, dst_name, dst_flags, spellid, spellname, spellschool, type, amount)
			elseif (token == "SPELL_AURA_REMOVED") then
				DpsTuningPlugin:AuraRemoved(time1, src_serial, src_name, src_flags, dst_serial, dst_name, dst_flags, spellid, spellname, spellschool, type, amount)
			elseif (token == "SPELL_AURA_REFRESH") then
				DpsTuningPlugin:AuraRefresh(time1, src_serial, src_name, src_flags, dst_serial, dst_name, dst_flags, spellid, spellname, spellschool, type, amount)
			end
		end
	...
	elseif (event == "ADDON_LOADED") then
		...
		if (AddonName == "Details_DpsTuning") then
			...
			if (_G._details) then
				...
				local install = _G._details:InstallPlugin("SOLO", "Dps Tuning", "Interface\\AddOns\\Details\\images\\Ability_Racial_RocketBarrage", DpsTuningPlugin, "DETAILS_PLUGIN_DPS_TUNING", MINIMAL_DETAILS_VERSION_REQUIRED, "Details! Team", "v1.01", default_settings)
